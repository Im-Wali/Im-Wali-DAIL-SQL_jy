...
# 함수 정의: get_sqls, 결과 데이터, 선택할 쿼리 수, 데이터베이스 디렉토리를 인자로 받음
def get_sqls(results, select_number, db_dir):
    db_ids = []          # 데이터베이스 ID를 저장할 리스트
    all_p_sqls = []      # 모든 선택된 SQL 쿼리를 저장할 리스트

    # results에서 각 항목에 대해 반복
    for item in results:
        p_sqls = []      # 임시로 SQL 쿼리를 저장할 리스트
        db_ids.append(item['db_id'])  # 현재 항목의 데이터베이스 ID를 db_ids에 추가

        # item의 'p_sqls' 필드에서 SQL 쿼리를 추출하고, select_number 만큼만 선택
        for i, x in enumerate(item['p_sqls']):
            p_sqls.append(x)
            if i+1 == select_number:
                break
        all_p_sqls.append(p_sqls)  # 선택된 쿼리를 all_p_sqls에 추가

    chosen_p_sqls = []    # 최종적으로 선택된 SQL 쿼리를 저장할 리스트

    # db_ids를 통해 데이터베이스 별로 반복 처리
    for i, db_id in enumerate(tqdm.tqdm(db_ids)):
        p_sqls = all_p_sqls[i]  # 현재 데이터베이스의 모든 선택된 SQL 쿼리
        db_path = f"{db_dir}/{db_id}/{db_id}"  # 데이터베이스 파일의 경로 생성

        cluster_sql_list = []  # 유사한 결과를 내는 SQL 쿼리의 클러스터를 저장할 리스트
        map_sql2denotation = {}  # SQL 쿼리와 그 결과를 매핑할 딕셔너리

        # p_sqls의 각 SQL 쿼리에 대해 실행 결과를 평가
        for sql in p_sqls:
            flag, denotation = get_exec_output(
                db_path,
                sql,
            )
            if flag == "exception":  # 예외 발생 시 해당 쿼리는 건너뜀
                continue
            map_sql2denotation[sql] = denotation  # SQL과 결과 매핑
            denotation_match = False

            # 이미 저장된 클러스터를 검사하여 현재 SQL 쿼리 결과와 일치하는지 확인
            for id, cluster in enumerate(cluster_sql_list):
                ...
                if result_eq(map_sql2denotation[center_sql], denotation, False):
                    ...
                    break
            if not denotation_match:  # 일치하는 클러스터가 없으면 새 클러스터 생성
        ...
        if not cluster_sql_list:
            chosen_p_sqls.append(p_sqls[0])  # 클러스터가 없다면 첫 번째 쿼리 선택
        else:
            chosen_p_sqls.append(cluster_sql_list[0][0])  # 가장 큰 클러스터의 첫 번째 쿼리 선택

    print("save chosen sqls and results...")  # 최종 선택된 SQL 쿼리 저장 메시지

    return chosen_p_sqls  # 최종 선택된 SQL 쿼리 반환
